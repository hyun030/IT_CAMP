...
<style>
    ...
    .connect-card {
        ...
        border-radius: 10px; background-color: white; height: 100%;
    }
    .connect-card .icon { width: 40px; height: 40px; margin: 0 auto 1rem auto; }
    .connect-card .info { flex-grow: 1; }
    .connect-card .info h3 { font-size: 1.2rem; margin-bottom: 0.2rem; margin-top: 0; }
    .connect-card .info p { color: var(--subtext-color); font-size: 0.9rem; margin-bottom: 1.5rem; }
    
    /* st.container(border=True)에 대한 추가 스타일 */
...
def show_input_page():
    ...
    st.markdown('<div style="margin-bottom: 2rem;"></div>', unsafe_allow_html=True) # 제목과 카드 사이 여백

    # <<<--- 여기를 수정했습니다 --- #
    col1, col2, col3 = st.columns([1, 2, 1]) # [왼쪽 여백, 중앙 콘텐츠, 오른쪽 여백]

    with col2:
        # st.container(border=True)를 사용해 입력 폼을 카드 형태로 만듭니다.
        with st.container(border=True):
            company = st.text_input("지원 회사명", placeholder="예: 삼성전자")
            job = st.text_input("지원 직무", placeholder="예: AI 연구원")
            
            st.markdown('<div style="margin-top: 1rem;"></div>', unsafe_allow_html=True) # 버튼 위 여백
...
def show_result_page():
    ...
    with st.container(border=False):
        ...
        for skill in USER_PROFILE['skills']:
            ...
            skills_html += f'<div class="skill-tag {highlight_class}">{skill}</div>'
